var fn = function(search) {
  ...
  "method": "POST"
}).then(response=> response.text()).then(data=>{     console.log(JSON.parse(data)['data']['xdt_api__v1__fbsearch__topsearch_connection']['users'].map(o => o['user']['username']).join('\r\n'));});
}
